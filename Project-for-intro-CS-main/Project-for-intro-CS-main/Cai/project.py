def build():  # 读取dict.dic，构建字典
    fhand = open("/home/caiciyu/Codefield/Py_code/Workspace/BUS/dict.dic", "r")  # 文件地址自行更改
...
def FCFS():  # 先来先服务策略
    ...
    for instruction in instructions:
        ...
        if instruction[0] == "clock":  # 如果是clock指令
            if(sum(bus.target)+sum(station.clockwise)+sum(station.counterclockwise) == 0):
                sign = 0
            if(sign!=0 and current_instruction[0]=="") or (sign!=0 and current_instruction[0]== "NULL"):
                current_instruction = (move_instructions[counter][0], move_instructions[counter][1], 1, 0)
                current_direction = select_direction(bus, current_instruction[1])
                counter = counter + 1
                current_instruction = (current_instruction[0], current_instruction[1], 1, current_direction)
                bus.change_direction(current_direction)
                sign = 0
            if(sum(bus.target)+sum(station.clockwise)+sum(station.counterclockwise) == 0):
                bus.change_direction(0)
            else:
                bus.change_direction(current_instruction[3])
            ...
            if current_instruction[2] == 0:  # 当前指令完成
                ...
                if counter != len(move_instructions):
                    while (current_instruction[1] == move_instructions[counter][1]):
                        ...
                        if(counter == len(move_instructions)):
                            break
                    if(counter == len(move_instructions)):
                        current_instruction = ("NULL", bus.position, 0, 0)
                        bus.change_direction(0)
                    else:
                        current_instruction = (move_instructions[counter][0], move_instructions[counter][1], 1, 0)
                        current_direction = select_direction(bus, current_instruction[1])
                ...
                if isdone(current_instruction):  # 判断当前指令是否完成
                    current_instruction = (current_instruction[0], current_instruction[1], 0, 0)
                    move_instructions[counter-1][2] = 0 #move_instruction的状态改为0
                    bus.change_direction(0)
            ...
            for move_instruction in move_instructions:
                if move_instruction[2] == 0:
                    continue
                if move_instruction[0] == instruction[0] and move_instruction[1] == int(instruction[1]):
                    insert_sign = 0
                    break
            if(insert_sign == 1):
                move_instructions.append([instruction[0], int(instruction[1]), 1])
...
def SCAN():  # 顺便服务策略
    ...
    def slect_move(move_instructions, bus): # 运行路线判断函数
        ...
        for first_move in range(num):
            dis, direct = distance(bus, move_instructions[first_move][1])
            if dis == mindistance and direct == 1:
                mindistance = dis
                request = move_instructions[first_move][0]
                address = move_instructions[first_move][1]
                final_direction = direct
    ...
    def determine_move(bus, station, current_instruction):
        if current_instruction[2] == 1:
            target_min = direction_distance(bus, station, 1, "target")
            clockwise_min = direction_distance(bus, station, 1, "clockwise")
            counterclockwise_min = direction_distance(bus, station, 1, "counterclockwise")
            if(target_min[1] == dict['DISTANCE'] * dict['TOTAL_STATION'] and counterclockwise_min[1] == dict['DISTANCE'] * dict['TOTAL_STATION'] and clockwise_min[1] == dict['DISTANCE'] * dict['TOTAL_STATION']):
                return 0, ("NULL",bus.position)
            if min(target_min[1], clockwise_min[1], counterclockwise_min[1]) * 2 > dict['DISTANCE'] * dict['TOTAL_STATION']:
                target_min = direction_distance(bus, station, -1, "target")
                clockwise_min = direction_distance(bus, station, -1, "clockwise")
                counterclockwise_min = direction_distance(bus, station, -1, "counterclockwise")
                max_list = [(target_min[0], target_min[1], "target"), (clockwise_min[0], clockwise_min[1], "clockwise"), (counterclockwise_min[0], counterclockwise_min[1], "counterclockwise")]
                max_list.sort(key=lambda x: x[1])
                return -1, (max_list[0][2], max_list[0][0])
    ...
    for instruction in instructions:
        ...
        if instruction[0] == 'clock':
            ...
            if(sign != 0 and current_instruction[0] == ""):  # 如果sign不为0的时候获得first_move_instruction
                ...
                move_instructions = []
            if(sign != 0 and current_instruction[0] == "NULL"):
                move_request, move_address, move_direction = slect_move(move_instructions, bus)
                current_instruction = (move_request, move_address, move_direction, 1)
                bus.change_direction(move_direction)
                move_instructions = []
            ...
            if current_instruction[3] == 0:  # 当前指令完成
                ...
                if(current_instruction[0] != "" and current_instruction[0] != "NULL"):
            ...
            if(sign == 0 or current_instruction[0]=="NULL"):
                sign = 1
                if(current_instruction[0] == "NULL"):
